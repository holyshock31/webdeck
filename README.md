...
通用桌面网页包装器：**通过 URL 添加应用**，每个应用可配置**启动方式**（自动拉起本地服务）与**健康监测**（状态灯）。侧边栏标签页形态，macOS 优先，Electron 实现。

## 快速开始

```bash
npm install        # 若 Electron 二进制下载失败，见下方「常见问题」
npm start          # 启动 WebDeck
npm test           # 核心逻辑单测（无需 GUI）
...
内置两个快捷预设：**DeepSeek Harness**（`pnpm dsh` → `http://127.0.0.1:3080`）和**本地静态服务**（`python3 -m http.server`）。
...
src/main/
  ...
  process-manager.js 本地进程生命周期：spawn(直接/Shell)、进程组终止、日志环形缓冲
  monitor.js        健康监测状态机：stopped/starting/running/error（纯 Node，可单测）
  store.js          JSON 原子持久化（userData/webdeck.json）
src/preload/
  preload.cjs       contextBridge 白名单 API（contextIsolation + sandbox）
src/renderer/
...
- **进程组管理**：本地命令以 `detached` 进程组启动，停止时 SIGTERM 整组、2 秒后 SIGKILL，不遗留子进程
...
- [ ] electron-builder 打包（dmg/zip，`appId` 待定）
- [ ] Windows 适配（进程组终止方式、打包脚本）
- [ ] 侧边栏可折叠 / 宽度可调
- [ ] 应用级代理配置、证书忽略开关
- [ ] 托盘图标 + 快捷切换
- [ ] 图标与品牌视觉
