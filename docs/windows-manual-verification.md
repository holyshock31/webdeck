...
执行完成后把结果（通过/失败项）写回本清单或提交到 `openspec/changes/support-cross-platform/verify.md`（归档前人工核验的验收记录）。
...
## 自动更新（add-update-check，v0.1.8 起）

- [ ] 菜单「帮助 → 检查更新…」手动检查：有新版本时出现更新提示条（含版本号），点击「查看」打开更新弹窗；无新版本时提示"已是最新版本"
- [ ] 自动检查：启动后约 5 秒静默检查，发现新版本出现提示条（不打扰无更新场景）
- [ ] 下载进度：提示条显示「正在下载更新… N%」
- [ ] 下载完成：提示条变为「新版本已就绪」，点击「立即安装」→ 应用退出 → 静默安装 → 自动重启进入新版本；点击「稍后」则退出应用时不安装
