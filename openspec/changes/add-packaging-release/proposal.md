...
- 打包态身份对齐：确认 `app.setAppUserModelId` 与 electron-builder `appId` 在打包态一致（统一 `com.webdeck.app`），保证 Windows 任务栏归属与通知正确
...
- **身份一致性**：`app.setAppUserModelId` 与 appId 统一为 `com.webdeck.app`，与 dev-mac.sh 的 CFBundleIdentifier 一致；打包态与开发态的应用身份不再有差异
