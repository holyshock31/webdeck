#!/bin/bash
# dev-mac.sh — 开发态运行入口（macOS）
#
# 直接用 `electron .` 跑开发态时，macOS 的 Dock 悬停名 / ⌘Tab 切换器显示的是
# "Electron"（取自 node_modules/electron/dist/Electron.app 包的 Info.plist，
# 运行时 API 无法修改，app.dock 没有改名的接口）。本脚本把该 .app 复制为
# dist/WebDeck.app，改写包内名称/标识/可执行文件名后再启动，使 Dock、⌘Tab、
# 菜单栏均显示 WebDeck。Electron 版本变化时自动重建副本。
# 非 macOS 平台直接回退到 `electron .`。
...
if [ ! -d "$DIST" ] || [ ! -f "$MARKER" ] || [ "$(cat "$MARKER" 2>/dev/null || true)" != "$SRC_VER" ]; then
  echo "[dev-mac] 生成 WebDeck.app（源 Electron ${SRC_VER}）…"
  rm -rf "$DIST"
  mkdir -p "$ROOT/dist"
  cp -R "$SRC" "$DIST"
